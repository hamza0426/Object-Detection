...
# # model = YOLO("yolo11n.pt")  # pretrained YOLOv8n

# # ---------- Helper function to classify size ----------
# def classify_size(box):
#     x1, y1, x2, y2 = map(int, box)
...
#     # ---------- Object counts ----------
#     object_count = {"small": 0, "medium": 0, "large": 0}
#     seen_ids = set()
#     frame_idx = 0

#     # Track objects using YOLO
#     results_stream = model.track(source=input_path, stream=True, persist=True, conf=0.3)
...
#                 size_category = classify_size(box)
#                 object_count[size_category] += 1

#                 # Draw rectangle and label
#                 x1, y1, x2, y2 = map(int, box)
#                 color_map = {"small": (0,255,0), "medium": (0,165,255), "large": (0,0,255)}
#                 color = color_map[size_category]

#                 cv2.rectangle(frame, (x1, y1), (x2, y2), color, 2)
#                 cv2.putText(
#                     frame,
#                     f"{names[int(cls_id)]} ID:{int(track_id)}",
#                     (x1, y1-10),
...
#     # ---------- Return JSON ----------
#     total = object_count["small"] + object_count["medium"] + object_count["large"]

#     return {
#         "small": object_count["small"],
#         "medium": object_count["medium"],
#         "large": object_count["large"],
#         "total": total,
#         "frame_count": frame_idx,
#         "timestamp": datetime.now().isoformat(),
#         "video_url": f"/outputs/{output_filename}",

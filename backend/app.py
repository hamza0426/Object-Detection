...
# Load YOLO once at startup — not per request
...
async def process_video(video: UploadFile = File(...)):
    ...
    # ── VideoWriter with mp4v (fast software codec) ──────────
    # mp4v is significantly faster than H264 on CPU — ideal for servers
    ...
    # ── Single-pass: track + write frames together ───────────
    # Key optimizations:
    #   - imgsz=416   : smaller inference size (faster, still accurate)
    #   - conf=0.45   : skip low-confidence detections
    #   - device='cpu': explicit CPU (avoids device detection overhead)
    ...
    for result in results:
        frame_idx += 1
        # Use the already-annotated frame from YOLO result
        # result.plot() draws boxes + labels in one optimized call
        # Much faster than manually drawing with cv2.rectangle + putText
        frame = result.plot()
        ...
        if result.boxes.id is not None:
            ...
            for cls_id, track_id in zip(
                ...
                object_count[label].add(int(track_id))

        out.write(frame)

    out.release()

    # ── Cleanup uploaded input to save disk space ────────────
    try:
